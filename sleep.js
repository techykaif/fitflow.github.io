...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function init() {
    ...
    get(sleepRecordsRef).then((snapshot) => {
        if (snapshot.exists()) {
            // If data exists, set it to sleepRecords array
            const sleepRecords = Object.values(snapshot.val()); // Converts object to array
            
            // Pass the fetched records to your render function
            renderSleepRecords(sleepRecords);
            
            // You can also pass sleepRecords to other functions like the chart and dashboard
            renderChart('weekly', sleepRecords);
            updateDashboardStats(sleepRecords);
            updateGoalProgress(sleepRecords);
        } else {
  ...
  function calculateSleepDuration() {
    ...
    let sleepDate = new Date(sleepDateInput.value);
    let sleepStart = new Date(sleepDate);
    let wakeTime = new Date(sleepDate);
  ...
  function provideSleepRecommendation(durationHours) {
    ...
    if (durationHours < 6) {
      ...
      recommendation = "'⚠️ You're sleeping more than typically recommended. If you feel excessively tired, consider consulting a doctor.'";
    }
  ...
  function updateSleepInsights(type) {
  ...
  function calculateAverageSleepQuality() {
  ...
  function saveSleepGoal() {
    const targetHours = parseFloat(sleepGoalHours.value);
    const targetDays = parseInt(sleepGoalDays.value);
    
    if (isNaN(targetHours) || isNaN(targetDays) || targetHours < 4 || targetHours > 12 || targetDays < 1 || targetDays > 7) {
